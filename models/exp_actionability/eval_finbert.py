...
def parse_args() -> argparse.Namespace:
    ...
    default_artifact_dir = project_dir / 'models' / 'artifacts' / 'finbert_actionability'

    parser = argparse.ArgumentParser(description='Evaluate the actionability FinBERT checkpoint on test split.')
...
def evaluate(test_df: pd.DataFrame, artifact_dir: Path,
    ...
    clf = pipeline(task='text-classification', model=str(artifact_dir), tokenizer=str(artifact_dir),
                   truncation=True, top_k=1)
