...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--label-mapping-path', type=Path, default=default_data_dir / 'actionability_label_mapping.json')
...
def build_peft_classifier(artifact_dir: Path, label2id: dict[str, int], id2label: dict[int, str]):
    peft_config = PeftConfig.from_pretrained(str(artifact_dir))

    base_model = AutoModelForSequenceClassification.from_pretrained(
        peft_config.base_model_name_or_path, num_labels=len(label2id), label2id=label2id,
    ...
    classifier = pipeline(task='text-classification', model=model,
                          tokenizer=tokenizer, truncation=True, top_k=1)
...
def evaluate(test_df: pd.DataFrame, classifier, ordered_labels: list[str],
    ...
    report = classification_report(true_labels, predicted_labels,
                                   labels=ordered_labels, output_dict=True, zero_division=0)
...
def main() -> None:
    ...
    metrics = evaluate(test_df=test_df, classifier=classifier,
                       ordered_labels=ordered_labels, batch_size=args.batch_size)
