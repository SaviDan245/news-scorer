...
def parse_args() -> argparse.Namespace:
    ...
    default_artifact_dir = project_dir / 'models' / 'artifacts' / 'finbert_event_type'

    parser = argparse.ArgumentParser(description='Evaluate the event type FinBERT checkpoint on test split.')
    parser.add_argument('--test-path', type=Path, default=default_data_dir / 'event_type_test.csv')
...
def load_test_split(path: Path) -> pd.DataFrame:
    ...
    df = df.dropna(subset=['text', 'event_type_label', 'label_id']).copy()
...
def evaluate(test_df: pd.DataFrame, artifact_dir: Path,
    ...
    true_labels = test_df['event_type_label'].str.strip().str.lower().tolist()
...
def main() -> None:
    ...
    metrics = evaluate(test_df=test_df, artifact_dir=artifact_dir,
